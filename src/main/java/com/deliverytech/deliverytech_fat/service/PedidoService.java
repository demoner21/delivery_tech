...
import com.deliverytech.deliverytech_fat.enums.*;
...
public class PedidoService {
    ...
    public Pedido adicionarItem(Long pedidoId, Long produtoId, Integer quantidade) {
        ...
        item.setSubtotal(BigDecimal.valueOf(item.getPrecoUnitario())
    ...
    public Pedido cancelarPedido(Long pedidoId, String motivo) {
        ...
        if (pedido.getStatus() != null && pedido.getStatus().equals(StatusPedido.ENTREGUE)) {
            throw new IllegalArgumentException("Pedido já entregue não pode ser cancelado");
        }

        if (pedido.getStatus() != null && pedido.getStatus().equals(StatusPedido.CANCELADO)) {
